...
def run(N):
    ...
    #        print(cluster)
        ...
        def steady_state_phase():
            ...
            for key, values in cluster.items():
                ...
                for x in values:
                    ...
                    packetloss = k * (1 - (dis / MaxDistance)) * (
                        1 - (member.energy / Node.energy_init)) * (1 - ((WSN.n-WSN.n_dead) / WSN.n))
                ...
                if len(values) > 0:
                    print(
                        f"""CH energy ==>{nodes[int(key)].energy}  \nNode 0th Energy ==>{nodes[int(values[0])].energy}""")
            ...
            print(
                f"\nTotal packet loss in this round ==> {math.floor((total_packetloss))}\n No. of dead nodes ==> {WSN.n_dead} \n No. of alive nodes ==> {WSN.n-WSN.n_dead}\n PDR ==> {((WSN.DM * (WSN.n-WSN.n_dead) - total_packetloss)/(WSN.DM * (WSN.n-WSN.n_dead) ))}  \n Total End-To-End Delay ==> {total_ETE_D/(WSN.n-WSN.n_dead):.5f}")
            print(
                f"""\n\n##########################################
    //////////////////////////////////////////
    \n
    Total END TO END DELAY ==> {total_ETE_D:.5f}\n

...
def run(N):
    ...
    class WSN(object):
        ...
        # Eelec = ETX = ERX
        # Energy for transferring of each bit:50nJ/bit
        ETX = 50 * (10 ** (-9))
        # Energy for receiving of each bit:50nJ/bit
        ERX = 50 * (10 ** (-9))
        # Transmit Amplifier types
        Efs = 10 * (10 ** (-12))     # Energy of free space model: 10pJ/bit/m2
        # Energy of multi path model: 0.0013pJ/bit/m4
        Emp = 0.0013 * (10 ** (-12))
    ...
    class Node(object):
        ...
        # PARAMETER 1 (ENERGY)
        energy_init = 0.2  # initial energy of a node
        ...
        def __init__(self):
            ...
            self.packets = WSN.DM
            # "N" = Node (Non-CH):The point type is a normal node
            self.type = "N"
    ...
    class Leach(object):
        ...
        p = 0.2    # Probability of being selected as a cluster head
                ...
                if nodes[i].energy > Node.energy_threshold:  # The node is not dead
                    if nodes[i].G == 0:  # The node is not selected as the cluster head in this cycle
                        ...
                        if temp_rand <= Tn:
                            ...
                            nodes[i].energy -= WSN.trans_energy(
                                WSN.CM, max_dis)
    ...
    #        print(cluster)
        ...
        def to_excel(data):
            workbook = openpyxl.load_workbook(
                'C:\\Users\\saily\\Documents\\WSN_LEACH-main\\sachi.xlsx')  # Load an existing file
        ...
        def end_to_end_delay(distance, delivered_packets, bitrate):
            # calculate the time it takes to transmit the packet
            tx_time = delivered_packets / bitrate

            # calculate the propagation delay based on the distance
            # assume the propagation speed is the speed of light
            propagation_delay = distance / 299792458.0

            # return the total end-to-end delay
            return tx_time + 2 * propagation_delay
        ...
        def steady_state_phase():
            ...
            for key, values in cluster.items():
                ...
                for x in values:
                    ...
                    # packetloss = (E_Packetloss/1.4 + (dis*1000)/4.3)*500 % WSN.DM
                    # member.packets -= packetloss

                    # formu 1
                    MaxDistance = np.sqrt(
                        np.power((WSN.xm), 2) + np.power((WSN.ym), 2))

                    # PL = k * distance_factor * Energy_factor * Node_factor
                    k = 10
                    packetloss = k * (1 - (dis / MaxDistance)) * (
                        1 - (member.energy / Node.energy_init)) * (20 - (5*((WSN.n-WSN.n_dead) / WSN.n)))  # 8 ==> 20 (12), 15 ==> 20 (5) ,20 ==> 25(5)
                    ...
                    ch_ETE_D += Leach.end_to_end_delay(dis,
                                                       (WSN.DM - packetloss), WSN.bitrate)
                ...
                total_ETE_D += ch_ETE_D

            pdr = ((WSN.DM * (WSN.n-WSN.n_dead) - total_packetloss) /
                   (WSN.DM * (WSN.n-WSN.n_dead)))

            # print(f"INITIAL PDR => {Leach.totalpdrfor_N}")

            Leach.totalpdrfor_N += pdr
            Leach.avg_EtoE_Delay_for_eachN += total_ETE_D/(WSN.n-WSN.n_dead)

            # print(f"PDR => {pdr}  avg PDR=> {Leach.totalpdrfor_N}")
            # print(Leach.totalpdrfor_N)
            # TO EXCEL
            # row = [f"{Leach.r}", f"{WSN.n_dead}", f"{WSN.n-WSN.n_dead}", f"{math.floor((total_packetloss))}",
            #        f"{((WSN.DM * (WSN.n-WSN.n_dead) - total_packetloss)/(WSN.DM * (WSN.n-WSN.n_dead) )):.5f}", f"{total_ETE_D/(WSN.n-WSN.n_dead):.5f}"]
            # Leach.to_excel(row)
        ...
        def run_leach():
            ...
            while WSN.flag_all_dead != 1:
                Leach.r += 1
                # print(Leach.r)
                # worksheet.append([Leach.r])

                nodes = WSN.nodes
                # G resets to 0 when a new cycle starts
                if (Leach.r % Leach.period) == 0:
                    # print("==============================")
                    for node in nodes:
                        node.G = 0
            ...
            # print(f"""
            # ==============================
            # ALL NODES DIED AT {Leach.r} th ROUND
            # ==============================""")

    def main():
        # Leach.to_excel([f"No. of nodes=> {N}"])
        # Leach.to_excel([""])
        # Leach.to_excel(["NEW ITERATION"])
        # Leach.to_excel([""])
        # Leach.to_excel(["Round no.", "No. of Dead Nodes", "No. of alive node",
        #                 "Packet loss", "PDR", "End to End Delay"])
        Node.init_nodes()
    ...
    print("Running")
    main()
    print(f"Total Rounds => {Leach.r}")
    print(f"Avg PDR for N nodes => {(Leach.totalpdrfor_N/Leach.r):.5f}")
    print(
        f"Avg End to End Delay of each node => {(Leach.avg_EtoE_Delay_for_eachN/Leach.r):.5f}")
    Leach.to_excel([f"{N}", f"{Leach.r}", f"{(Leach.totalpdrfor_N/Leach.r):.5f}",
                   f"{(Leach.avg_EtoE_Delay_for_eachN/Leach.r):.5f}"])
